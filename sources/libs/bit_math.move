module libs::bit_math {
    ...
    /// @param x_input the value for which to compute the most significant bit, must be greater than 0
    /// @return the index of the most significant bit
    public fun get_most_significant_bit(x_input: u256): u8 {
        assert!(x_input > 0, E_UNDERFLOW);
    ...
    /// @param x_input the value for which to compute the least significant bit, must be greater than 0
    /// @return r the index of the least significant bit
    public fun get_least_significant_bit(x_input: u256): u8 {
        assert!(x_input > 0, E_UNDERFLOW);
    ...
    public fun test_get_most_significant_bit() {
        ...
        let mut i = 0;
        while(i < 255) {
            assert!(get_most_significant_bit(std::u256::pow(2, i)) == i, 0);
            i = i + 1;
        };

        //uint256 max
        assert!(get_most_significant_bit(libs::constants::get_max_u256()) == 255, 0);
    }
    ...
    public fun test_get_least_significant_bit() {
        ...
        let mut i = 0;
        while(i < 255) {
            assert!(get_least_significant_bit(std::u256::pow(2, i)) == i, 0);
            i = i + 1;
        };

        //uint256 max
        assert!(get_least_significant_bit(libs::constants::get_max_u256()) == 0, 0);
    }
}